export const SKILL_NAME = "runpulse-supervisor";

export const SKILL_TEMPLATE = `---
name: runpulse-supervisor
description: Active supervisor for a runpulse autonomous run. Use when the user asks to supervise, babysit, watch over, or keep alive a long agent run, when they mention a stalled or overnight run, or when a .runpulse/ run is in progress. Performs one supervision cycle - gather signals, apply a bounded intervention playbook, report - and is meant to be repeated with /loop 10m.
---

# runpulse supervisor

You are the **active supervisor** of the autonomous run in \`.runpulse/\`. A separate runner process
(\`runpulse run\`) launches one headless agent session per milestone. That session owns the project
...
- \`tail -n 20 .runpulse/run-log.md\` - what the engine did (launches, verdicts, infra waits, kills).
- \`tail -n 10 .runpulse/supervisor-log.md\` - what *you* already did. Read it before acting: it is
...
- The last entry of \`.runpulse/execution-log.md\` and any new \`.runpulse/decisions.md\` entries.
...
exists)
Run \`runpulse attend\`. That runs the project's configured environment adapter (window focus,
...
minutes)
\`runpulse kill --reason "<what you observed>"\`. The runner grades the killed session as
...
Rules 3, 4 and 6 change the world. \`runpulse kill\` and \`runpulse attend\` write their own line to
\`.runpulse/supervisor-log.md\`. For anything else you do (a relaunch, a decision to hold), append
one line yourself:
