...
export function steer(options: SteerOptions): number {
  ...
  if (options.text === undefined) {
    ...
    if (!current) {
      ...
      info(`  set some with: ${color.bold('milestoner steer "prefer the simpler fix over the general one"')}`);
      return 0;
    }
