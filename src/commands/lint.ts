...
export function collectLintInput(config: MilestonerConfig, layout: Layout, state: RunState): LintInput {
  return {
    run: state.run,
...
export function lintTotals(findings: LintFinding[]): { errors: number; warnings: number; summary: string } {
...
export function lint(options: LintOptions): number {
  ...
  const { errors, warnings, summary } = lintTotals(findings);
