...
// The panel lookup resolves against the machine directory; keep these tests out of the real one.
process.env.MILESTONER_HOME = mkdtempSync(join(tmpdir(), "milestoner-runs-home-"));
...
async function capture(fn: () => Promise<number>): Promise<{ code: number; out: string }> {
  ...
  try {
    return { code: await fn(), out: plain(lines.join("\n")) };
  } finally {
...
test("both runs are printed with their project, milestone, progress and verdict", async () => {
  const file = registry();
  ...
  const { code, out } = await capture(() => runs({ registry: file, json: false }));
...
test("a blocked run exits 2, the same signal `status` gives", async () => {
  const file = registry();
...
test("a deleted project is reported as pruned and does not take the listing down with it", async () => {
  const file = registry();
...
test("--json is the same listing, machine-readable", async () => {
  const file = registry();
  ...
  const { out } = await capture(() => runs({ registry: file, json: true }));
...
test("an empty registry says so and exits 0", async () => {
  const { code, out } = await capture(() => runs({ registry: registry(), json: false }));
