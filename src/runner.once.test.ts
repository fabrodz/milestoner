...
import { PULSEFLOW_DIR, layoutFor } from "./paths.js";
...
function scaffold(verdict: unknown) {
  const root = mkdtempSync(join(tmpdir(), "pulseflow-once-"));
  ...
  config.agent = {
    ...
    args: [join(root, "agent.mjs"), "{{milestoneId}}", join(root, PULSEFLOW_DIR), join(root, "verdict.json")],
    modelArgs: [],
    model: null,
    env: {},
  };
