...
- `pulseflow run --once` exited `0` on a milestone that reported blocked, contradicting the
...
- `pulseflow report`: a single self-contained HTML report of the run, with a wall-clock timeline.
- `pulseflow steer`: mid-flight corrections injected into every session launched from then on.
...
- The active supervisor as an installable Claude Code skill (`pulseflow skill install`).
- `pulseflow kill` and `pulseflow attend`, the supervisor's only interventions, both logged.
- The environment adapter as a config string.

### Changed

- Renamed from `runpulse` to `pulseflow`. A `.runpulse/` directory is detected and the CLI explains
