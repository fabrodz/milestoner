...
## What runpulse actually does

You split the work into milestones and hand-write a spec for each one. `runpulse run` then loops:
...
4. When the session exits, read `.runpulse/result.json` (the session's own verdict), **grade it**,
...
```mermaid
flowchart TD
  A[runpulse run] --> B{next milestone<br/>not done?}
...
- Success cannot be checked without you looking at it. runpulse cannot grade taste.
- The acceptance criteria are still unknown. Explore first, then write milestones.

## Install and requirements

```sh
npm install -g runpulse
# or, per project:
npx runpulse init
```
...
symptom, everything it tried, and the single clearest action for you. `runpulse status` prints that
diagnosis; `runpulse unblock <id>` puts the milestone back in play once you have fixed the cause.
...
runpulse init --run checkout-v2 --milestones 4
```
...
```
  ...
  decisions.md
  supervisor-log.md
== initialized .runpulse/ for run "checkout-v2"

Next:
  1. Edit .runpulse/protocol.md - replace every TODO with this project's rules.
  2. Write .runpulse/prompts/M01.md and friends - objective, tasks, acceptance criteria, exit.
...
Open `.runpulse/protocol.md` and replace every `TODO`. This is the file every session reads first, so
...
`.runpulse/prompts/M01.md` and friends. This is the actual work specification and runpulse never
...
== M01 - Cart schema + migration  (attempt 1/3)
   prompt     .runpulse/prompts/M01.md
   transcript .runpulse/logs/M01-20260818-2312.log
...
.runpulse/
  ...
  kill.json            transient marker written by `runpulse kill`
...
Filled-in example, `.runpulse/prompts/M02.md`:
...
- `.runpulse/result.json` written with `status: "done"` and one evidence line per criterion.
```
...
Run it from the project root or any subdirectory: runpulse walks up looking for
`.runpulse/config.json`, the way git finds `.git`.

**Ctrl-C once**: the runner stops after the current session finishes and leaves the milestone
`in_progress`; a later `runpulse run` picks it up and starts a fresh attempt. **Ctrl-C twice**: the
...
```sh
runpulse status >/dev/null || notify-send "runpulse needs you"
```
...
```json
{
  ...
  "pulse": {
    ...
    "agentPid": 24512,
    "transcript": ".runpulse/logs/M03-20260818-2342.log",
...
```sh
runpulse unblock <milestoneId> [--keep-attempts]
```
...
Writes the supervisor skill to `.claude/skills/runpulse-supervisor/SKILL.md` in the project, or to
...
Before killing, it writes `.runpulse/kill.json`. Without that marker a session killed after a quiet
...
`.runpulse/config.json`, created by `init`:
...
| `environment.attendCommand` | `null` | Shell command line run by `runpulse attend`, with a `{{seconds}}` placeholder. `null` disables the intervention. |
...
`{{projectRoot}}`, `{{runpulseDir}}`, `{{model}}`. An unknown placeholder is left untouched so you can
...
happening. The recursive scan skips `node_modules`, `.git`, `.runpulse`, `dist`, `Library`, `Temp`,
`obj`, `bin` and dot-entries, and goes six levels deep.

## How the engine grades a session

The session writes `.runpulse/result.json` before exiting:
...
| Session killed by `runpulse kill` | incomplete | yes, always | `pending` (retry) |
...
1. it was not killed by `runpulse kill`, and
...
The pulse block in `runpulse status` answers three separate questions.
...
- `pulse.json` present and the pid dead means the runner died abruptly (closed terminal, reboot, OOM).
  Relaunch with `runpulse run`; the milestone is picked up again.
...
```sh
while true; do clear; runpulse status; sleep 300; done
```
...
```sh
runpulse skill install          # writes .claude/skills/runpulse-supervisor/SKILL.md
```
...
Its entire write surface is four things: `runpulse kill`, `runpulse attend`, relaunching
`runpulse run`, and appending to `.runpulse/supervisor-log.md`.
...
can corrupt the run. And it never calls `runpulse unblock`: clearing a block is a human decision.
...
| 3 | A watched signal frozen past its normal cadence and no fresher one | `runpulse attend`, then re-check next cycle. Cannot fire without an adapter. |
| 4 | An agent process exists but every signal is older than 25 minutes | `runpulse kill --reason "..."`. Twice on the same milestone means escalate instead. |
...
| 6 | Work remains, nothing blocked, no runner process | Relaunch `runpulse run`. Two failed relaunches in a row means escalate. |
...
`runpulse status --json` to see the run, `runpulse kill` for a hung session, `runpulse attend` for a
wedged environment, `runpulse run` to relaunch a dead runner.
...
A headless project leaves `attendCommand` at `null`. Then `runpulse attend` fails with an explanation
...
```sh
runpulse init --run checkout-v2 --milestones 5
...
```sh
runpulse run 2>&1 | tee run-checkout-v2.txt
```

In the morning, `runpulse status`. Three outcomes are possible and all three are useful.

- `5/5 done`: read `execution-log.md` and `decisions.md`, then review the five tags.
- `3/5 done, 1 blocked`: the diagnosis says what to fix. Then `runpulse unblock M04 && runpulse run`.
- `runner gone`: the machine slept or the terminal closed. `runpulse run` resumes from where the state
...
The case that made runpulse exist. The editor must run on your machine with a real window, so
...
```sh
runpulse init --run port-to-v2 --milestones 15
for i in $(seq -w 2 15); do sed "s/M01/M$i/" .runpulse/prompts/M01.md > .runpulse/prompts/M$i.md; done
...
```sh
runpulse run --milestone M01 --once
runpulse status
```

If M01 comes back `done` with evidence that convinces you, launch the rest with plain `runpulse run`.
...
```sh
runpulse run --milestone M01 --model claude-sonnet-5 --once
runpulse run --milestone M02 --model claude-opus-5 --once
```
...
You do not have to leave. `--once` turns runpulse into a disciplined single-shot runner: fresh
session, hand-written spec, evidence gate, archived claim, and you review between milestones.

```sh
runpulse run --once && runpulse status && git log --oneline -5
```
...
```sh
runpulse skill install
runpulse run           # terminal 1, or a detached shell
```
...
the morning `runpulse status` plus `supervisor-log.md` tell you the whole story.

## Recipes

**Resume after anything.** `runpulse run`. There is no separate resume command; the state file is the
...
its `status` to `"pending"`, `attempts` to `0`, and clear `evidence`. Then `runpulse run --milestone M03`.
...
```sh
runpulse run --milestone M04 --model claude-opus-5
```

**Give one milestone more attempts.**

```sh
runpulse run --milestone M04 --max-attempts 6
```

**Restart a session that is clearly stuck**, without stopping the run:

```sh
runpulse kill --reason "no test output for 40m, transcript still empty"
```

**Read the playbook before installing the skill.**

```sh
runpulse skill install --print | less
```
...
**Read what a session actually claimed.** `.runpulse/results/M03-attempt2.json` is the raw, ungraded
...
`tail -f` on the log is usually silent. Watch the liveness paths instead, or `runpulse status`.
...
case $code in
  0) echo "run complete" ;;
  2) echo "blocked: $(runpulse status --json | jq -r '.milestones[] | select(.status=="blocked") | .id')" ;;
  *) echo "runner error" ;;
esac
```
...
```sh
cat .runpulse/supervisor-log.md
grep killed .runpulse/run-log.md
```
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| `no .runpulse/config.json found here or in any parent directory` | You are outside the project. | `cd` into it, or run `runpulse init`. |
...
| A milestone is blocked with no diagnosis | The session hit the wall and gave up without writing one. | Read the transcript in `.runpulse/logs/`, fix the cause, then `unblock`. |
...
| `possibly hung` with a very long session | The agent is stuck in a loop or waiting on something. | `runpulse kill --reason "..."`. The runner consumes the attempt and relaunches with fresh context. |
| `runner gone (pid ... not running)` | The runner process died abruptly. | `runpulse run` to resume. For overnight runs use `tmux`/`screen`, disable sleep, and let the supervisor's rule 6 handle it. |
...
**Can two runs share a project directory?** Not in v0.2: the paths under `.runpulse/` are fixed. Use
...
session and then review them. What runpulse refuses to do is generate them silently as part of a run.
