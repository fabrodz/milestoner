...
The environment adapter is the one piece dogwatch cannot supply for you. Unsticking a host is
...
`dogwatch attend` runs whatever `environment.attendCommand` names, and playbook rule 3 is the only
...
2. **Print one line per thing it did.** The last ten lines are shown; the last one lands in
   `.dogwatch/supervisor-log.md`, so make it a summary.
